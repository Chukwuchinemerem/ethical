...
class Investment(models.Model):
    ...
    def daily_profit(self):
        return (self.amount * self.tier.roi_percentage / Decimal('100')) / self.tier.duration_days

    def progress_percent(self):
        from datetime import timedelta
        total_days = self.tier.duration_days
        elapsed = (timezone.now() - self.start_date).days
        return min(100, int((elapsed / total_days) * 100)) if total_days > 0 else 0

    def days_remaining(self):
        from datetime import timedelta
        end = self.start_date + timedelta(days=self.tier.duration_days)
        remaining = (end - timezone.now()).days
